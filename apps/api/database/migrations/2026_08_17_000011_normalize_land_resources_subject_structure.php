...
return new class extends Migration
{
    ...
    private function normalizeSubjects(array $sections): array
    {
        foreach ($sections as $index => $section) {
            ...
            $masterStart = preg_match('/\b(Master subjects|Master\'?s?\s+Degree|Дисциплины магистратуры|Magistratura fanlari|مواد الماجستير)\s*:?/iu', $text, $match, PREG_OFFSET_CAPTURE)
                ? $match[0][1]
                : false;
    ...
    {
        $text = preg_replace('/\b(Bachelor subjects|Bachelor programs|Bachelor\'?s?\s+Degree|Bakalavriat fanlari|Дисциплины бакалавриата|مواد البكالوريوس|Master subjects|Master programs|Master\'?s?\s+Degree|Magistratura fanlari|Дисциплины магистратуры|مواد الماجستير)\s*:?/iu', '', $text);
