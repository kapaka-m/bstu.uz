    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl border border-gray-100 bg-white p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-black text-navy">Interactive Services CMS</h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={fetchItems} className="inline-flex items-center gap-2 rounded-xl border border-gray-100 px-4 py-2 text-sm font-bold text-gray-600">
      ...
      {activeTab === "items" ? (
        <div className="grid gap-6">
          <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
            <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <button onClick={startCreate} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-extrabold text-white">
                      ...
                      return (
                        <tr key={item.id} className="align-top">
                          <td className="px-4 py-4">
                            <div className="font-extrabold text-navy">{en?.title || item.slug}</div>
                            <div className="mt-1 text-xs font-semibold text-gray-400">{item.slug}</div>
                          </td>
                          <td className="max-w-xs truncate px-4 py-4 text-xs font-semibold text-gray-500">{item.url}</td>
                          <td className="px-4 py-4">
                            <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${item.home_visible ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>
                              {item.home_visible ? "Visible" : "Hidden"}
                            </span>
                          </td>
                          <td className="px-4 py-4">
                            <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${item.is_active ? "bg-primary/10 text-primary" : "bg-red-50 text-red-700"}`}>
                              {item.is_active ? "Published" : "Draft"}
                            </span>
                          </td>
                          <td className="px-4 py-4">
                            <div className="flex justify-end gap-2">
                              <button onClick={() => startEdit(item)} className="rounded-lg border border-gray-100 p-2 text-gray-500 hover:text-primary">
                                <Edit3 className="h-4 w-4" />
                              </button>
                              <button onClick={() => togglePublish(item)} className="rounded-lg border border-gray-100 p-2 text-gray-500 hover:text-primary">
          ...
          {isFormOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/40 p-4 backdrop-blur-sm">
              <form onSubmit={handleSubmit} className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl border border-gray-100 bg-white p-6 shadow-2xl">
                <div className="mb-5 flex items-center justify-between">
                  <div>
                    <p className="text-xs font-extrabold uppercase tracking-widest text-primary">Service</p>
                    <h2 className="text-lg font-black text-navy">{editingRecord ? "Edit Service" : "Add Service"}</h2>
                  </div>
                  <button type="button" onClick={closeForm} className="rounded-lg p-2 text-gray-400 hover:bg-gray-50">
                    <X className="h-4 w-4" />
                  </button>
                </div>

                <datalist id="interactive-service-icons">
                  {icons.map((icon) => <option key={icon} value={icon} />)}
                </datalist>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  <label className="text-xs font-bold text-gray-500 lg:col-span-2">
                    URL
                    <input className={`${inputClass()} mt-1`} value={form.url} onChange={(event) => setForm((current) => ({ ...current, url: event.target.value }))} />
                  </label>
                  ...
                  <label className="text-xs font-bold text-gray-500">
                    ...
                    <input
                      ...
                      className={`${inputClass()} mt-1`}
                      value={form.icon}
                      onChange={(event) => setForm((current) => ({ ...current, icon: slugify(event.target.value) }))}
                    />
                  </label>
                  ...
                  <label className="text-xs font-bold text-gray-500">
                    ...
                    <input
                      ...
                      className={`${inputClass()} mt-1`}
                      value={form.color}
                      onChange={(event) => setForm((current) => ({ ...current, color: slugify(event.target.value) }))}
                    />
                  </label>

                  <label className="text-xs font-bold text-gray-500">
                    Order
                    <input type="number" className={`${inputClass()} mt-1`} value={form.sort_order} onChange={(event) => setForm((current) => ({ ...current, sort_order: event.target.value }))} />
                  </label>

                  <label className="text-xs font-bold text-gray-500">
                    Home
                    <span className={toggleClass()}>
                      <input type="checkbox" checked={form.home_visible} onChange={(event) => setForm((current) => ({ ...current, home_visible: event.target.checked }))} />
                      Show on home
                    </span>
                  </label>
                  ...
                  <label className="text-xs font-bold text-gray-500">
                    Opens New Tab
                    <span className={toggleClass()}>
                      <input type="checkbox" checked={form.opens_new_tab} onChange={(event) => setForm((current) => ({ ...current, opens_new_tab: event.target.checked }))} />
                      External tab
                    </span>
                  </label>
                  ...
                  <label className="text-xs font-bold text-gray-500">
                    Title
                    <input className={`${inputClass()} mt-1`} value={currentTranslation.title} onChange={(event) => updateTranslation(activeLocale, "title", event.target.value)} />
                  </label>
                  <label className="text-xs font-bold text-gray-500">
                    Button Label
                    <input className={`${inputClass()} mt-1`} value={currentTranslation.action_label} onChange={(event) => updateTranslation(activeLocale, "action_label", event.target.value)} />
                  </label>
                  <label className="text-xs font-bold text-gray-500 lg:col-span-3">
                    Description
                    <textarea className={`${inputClass()} mt-1 min-h-24`} value={currentTranslation.description} onChange={(event) => updateTranslation(activeLocale, "description", event.target.value)} />
                  </label>

                  <button disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-extrabold text-white disabled:opacity-60 lg:col-span-3">
        ...
        <form onSubmit={handleSettingsSubmit} className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
          <div className="mb-6 grid gap-4 md:grid-cols-[180px_180px_1fr]">
            <label className="text-xs font-bold text-gray-500">
              Home Limit
              <input type="number" min="1" max="24" className={`${inputClass()} mt-1`} value={settingsForm.home_limit} onChange={(event) => setSettingsForm((current) => ({ ...current, home_limit: Number(event.target.value) }))} />
            </label>
            <label className="flex min-h-10.5 items-center gap-2 self-end rounded-xl border border-gray-100 px-3 py-2.5 text-sm font-bold text-navy">
              <input type="checkbox" checked={settingsForm.is_active} onChange={(event) => setSettingsForm((current) => ({ ...current, is_active: event.target.checked }))} />
              Active
            </label>
          </div>
            ...
            {Object.keys(emptySettingsTranslation).map((key) => (
              <label key={key} className="text-xs font-bold text-gray-500">
                ...
                <input
                  ...
                  onChange={(event) => updateSettingTranslation(activeLocale, key, event.target.value)}
                />
              </label>
            ))}
          </div>

          <button disabled={savingSettings} className="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-extrabold text-white disabled:opacity-60">
