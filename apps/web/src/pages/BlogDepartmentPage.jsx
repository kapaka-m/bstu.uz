  ...
  if (!department) {
    return (
      <div className="pt-24 min-h-screen bg-primary-light flex flex-col items-center justify-center text-center px-4">
        ...
        <Link to="/blog" className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-extrabold text-white">
          {t("nav.blog")}
        </Link>
      </div>
    );
    ...
    <div className="pt-20 bg-white" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <section className="container mx-auto px-4 md:px-8 max-w-7xl py-12 md:py-16">
        <div className="mb-8 flex items-center justify-between gap-4">
          <h2 className="text-2xl font-extrabold text-navy">{t("nav.blog")}</h2>
          <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-extrabold text-primary">
            {department.blogs?.length || 0}
          </span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {(department.blogs || []).map((post) => (
            <article key={post.slug || post.id} className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm text-start">
              {post.image && (
                <Link to={`/blog/${post.slug}`} className="block mb-5 overflow-hidden rounded-2xl bg-gray-50 aspect-16/10">
                  <img src={post.image} alt={asText(post.title)} className="h-full w-full object-cover transition-transform duration-500 hover:scale-105" />
                </Link>
              )}
              ...
              <h3 className="mb-3 text-lg font-extrabold leading-snug text-navy hover:text-primary">
                <Link to={`/blog/${post.slug}`}>{asText(post.title)}</Link>
              </h3>
              <p className="mb-5 line-clamp-3 text-sm leading-relaxed text-gray-500">
                {asText(post.excerpt)}
              </p>
              <Link to={`/blog/${post.slug}`} className="inline-flex items-center gap-1.5 text-xs font-extrabold text-primary">
                {t("common.readMore")}
