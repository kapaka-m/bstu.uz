...
export default function GreenCampusDetails() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(() => {
        ...
        setArticle(null);
        setArticles([]);
        setError(t("common.loadError") || "Unable to load green campus article.");
      })
      .finally(() => {
  ...
  if (loading) {
    return (
      <div className="pt-24 bg-white min-h-screen overflow-x-hidden" dir={isRtl ? "rtl" : "ltr"}>
        <div className="container mx-auto px-4 md:px-8 max-w-7xl py-12 md:py-16 min-w-0">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 min-w-0">
            <div className="lg:col-span-8 flex min-w-0 flex-col gap-6">
              ...
              <div className="h-10 w-full max-w-xl rounded-full bg-gray-100 animate-pulse" />
            ...
            <div className="lg:col-span-4 hidden min-w-0 lg:flex flex-col gap-6">
              {[1, 2, 3].map((item) => (
  ...
  const publisherName = asText(publisher?.name || article.author);
  const publisherRoute = publisher?.slug ? `/publishers/${publisher.slug}` : "";

  return (
    <div className="pt-24 bg-white text-start min-h-screen overflow-x-hidden" dir={isRtl ? "rtl" : "ltr"}>
      {lightboxIndex !== null && (
        <GalleryLightbox
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-7xl py-12 md:py-16 min-w-0">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start min-w-0">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6 min-w-0">
            ...
            {/* Meta tags */}
            <div className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2 text-xs font-semibold text-gray-400 mt-2">
              ...
              <span className="flex items-center gap-1.5 min-w-0">
                ...
                {publisherRoute ? (
                  <Link to={publisherRoute} className="min-w-0 hover:text-emerald-600 transition-colors break-words">
                    {publisherName}
                  </Link>
                ) : (
                  <span className="break-words">{publisherName}</span>
                )}
              </span>
              {article.views && (
                <span className="flex items-center gap-1.5">
                  <Eye className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                  <span className="truncate">{article.views.toLocaleString(language)} {settings.views_label || ""}</span>
                </span>
              )}
            </div>

            {/* Main Title */}
            <h1 className="text-2xl md:text-3xl font-extrabold text-navy leading-tight break-words">
              {asText(article.title)}
            </h1>

            {/* Paragraphs */}
            <div className="flex flex-col gap-5 text-gray-600 text-xs md:text-sm font-semibold leading-relaxed break-words">
              {(Array.isArray(article.paragraphs) && article.paragraphs.length > 0
                ? article.paragraphs
                : [article.excerpt]
              ).filter(Boolean).map((para, idx) => (
                <p key={idx}>{asText(para)}</p>
              ))}
            </div>
                    ...
                    >
                      ...
                      <div className="p-4">
                        <span className="text-[9px] text-emerald-600 font-bold uppercase tracking-wider">
                          {categoryLabel(asText(art.category))}
                        </span>
                        <h4 className="text-sm font-extrabold text-navy group-hover:text-emerald-600 transition-colors leading-snug mt-1 line-clamp-2 break-words">
                          {asText(art.title)}
                        </h4>
                      </div>
                    </Link>
              ...
              <div className="relative min-w-0">
                <Link to="/green-campus" className="block">
                  <input
                    ...
                    className="w-full min-w-0 ps-10 pe-4 py-3 rounded-xl bg-white border border-gray-200 cursor-pointer text-sm font-semibold transition-colors"
                  />
                ...
                {categories.map((cat) => (
                  <li key={cat.value}>
                    <Link
                      ...
                      className="w-full flex min-w-0 items-center justify-between gap-3 py-1.5 transition-all text-start text-gray-500 hover:text-emerald-600"
                    >
            ...
            <div className="bg-gray-50 border border-gray-100 p-5 sm:p-8 rounded-3xl min-w-0">
              ...
              <div className="flex flex-col gap-4">
                {recentArticles.map((art) => (
                  <div key={art.id} className="flex min-w-0 gap-4 items-start group">
                    {art.image ? (
                      <img
                        src={art.image}
                        alt={asText(art.title)}
